...
let lapTime = 0; // время круга в секундах
let currentLap = 1; // номер круга
let speed = 50; // начальная скорость (в км/ч)

function updateStats() {
    // Обновляем статистику на экране
    lapTime += 0.1; // увеличиваем время на каждую анимацию
    if (lapTime >= 60) { // если время круга больше 60 секунд
        lapTime = 0; // сбрасываем время
        currentLap += 1; // увеличиваем номер круга
    }
...
// Функция для форматирования времени (чч:мм:сс)
...
// Обновляем статистику каждые 100 миллисекунд
